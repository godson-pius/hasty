...
/*
  ...
  Plugin URI: https://www.cyberseo.net/cybersyn/
...
if (isset($_GET['rate_cyberseo_lite'])) {
    update_option('cxxx_rated', 1);
    header('Location: https://wordpress.org/support/plugin/cybersyn/reviews/?filter=5');
    exit();
}
...
define('CSYN_CURL_USER_AGENT', 'Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)');
...
function csyn_preset_options() {
    ...
    if (get_option(CSYN_CANONICAL_LINK) === false) {
        update_option(CSYN_CANONICAL_LINK, '');
    }

    if (get_option(CSYN_LINK_TO_SOURCE) === false) {
        update_option(CSYN_LINK_TO_SOURCE, '');
    }
...
class CyberSyn_Syndicator {
    ...
    function parseFeed($feed_url) {
        ...
        if (!is_array($rss_lines) || stripos($rss_lines[0], '<?xml') === false) {
            $rss_lines = csyn_file_get_contents($feed_url, true, CSYN_CURL_USER_AGENT);
        }
    ...
    function displayPost() {
        ...
        echo '<strong>Post example:</strong><br />';
        echo '<div style="overflow:auto; max-height:350pt; border:1px #ccc solid; background-color:white; padding:8px; margin:8px 0 8px; 0;">';
                ...
                <td>
                    ...
                    if (!$result) {
                        if (!is_object($feed_url)) {
                            echo '<div id="message"><p><strong>No feed found at</strong> <a href="https://validator.w3.org/feed/check.cgi?url=' . urlencode($feed_url) . '" target="_blank">' . htmlspecialchars($feed_url) . '</a><br />';
                ...
                if (count($post_ids) > 0) {
                    foreach ($post_ids as $post_id) {
                        if (get_option(CSYN_KEEP_IMAGES) == "on") {
                            csyn_delete_post_images($post_id);
                        }
    ...
    function insertPost() {
        ...
        if ($this->current_feed['options']['embed_videos'] == 'on') {
            ...
            if (strpos($this->post['link'], '.youtube.com/') !== false || strpos($this->post['link'], '//youtube.com/') !== false) {
                ...
                $content = csyn_file_get_contents($this->post['link'], false, CSYN_CURL_USER_AGENT);
                preg_match('/"url":"(https:.*?\.mp4)","width":1920,"height":1080,/', $content, $matches);
                if (isset($matches[1]) && strlen($matches[1])) {
                    if (strpos($matches[1], '{') > 0) {
                        preg_match('/(https:.*?\.mp4)"/', $matches[1], $matches);
                    }
            ...
            } elseif (strpos($this->post['guid'], '.ustream.tv/') !== false || strpos($this->post['guid'], '//ustream.tv/') !== false) {
                $this->post['post_content'] = '<iframe width="480" height="270" src="' . str_replace('/recorded/', '/embed/recorded/', $this->post['guid']) . '" scrolling="no" allowfullscreen webkitallowfullscreen frameborder="0" style="border: 0 none transparent;"></iframe>' . "\n<br />" . strip_tags($this->post['post_excerpt'], '<br>,<b>,<p>,<a>');
            }
        }
    ...
    function showMainPage($showsettings = true) {
        ...
        if (!get_option('cxxx_rated')) {
            echo '<div id="message" class="notice is-dismissible updated fade" style=""><span style="display: block; margin: 0.5em; clear: both;">';
            echo 'If you are enjoying CyberSEO Lite, would you please mind to <a href="?rate_cyberseo_lite" target="_blank">give us a 5 stars</a> (⭐⭐⭐⭐⭐) review? Your valuable review will inspire us to make the even better!</div>';
        }
        if (defined('CSYN_SPINNER_VERSION') && CSYN_SPINNER_VERSION < 121) {
            echo '<div id="message" class="notice is-dismissible updated fade" style=""><span style="display: block; margin: 0.5em; clear: both;">';
            echo 'Your sipinner.php file is outdated. Please replace it with the latest version with <a href="https://www.cyberseo.net/partners/tbs.php" target="_blank"><strong>TBS</strong></a> support. Download <a href="https://www.cyberseo.net/downloads/spinners.zip">spinners.php</a> now.</div>';
        }
...
function csyn_options_menu() {
    ...
    if (isset($_POST['Submit'])) {
        ...
        $update_csyn_queries[] = update_option(CSYN_KEEP_IMAGES, csyn_sanitize_checkbox(@$_POST[CSYN_KEEP_IMAGES]));
        ...
        $update_csyn_text[] = __('Eeep images');
    ...
    <div class="wrap">
        ...
        <form method="post" action="<?php echo csyn_REQUEST_URI(); ?>" name="general_settings">
            ...
            <table class="form-table">
                ...
                <tr valign="top">
                    <th align="left">Keep downloaded images</th>
                    <td align="left"><input type="checkbox" name="<?php echo CSYN_KEEP_IMAGES; ?>"
                        <?php
                        if (get_option(CSYN_KEEP_IMAGES) == "on") {
                            echo "checked";
                        }
                        ?> /> keep all the downloaded and attached images when post is permanently deleted on "Empty Trash".</td>
                </tr>                     
...
function cyberseo_lite_upgrade_to_pro_menu() {
    // empty
}

function cyberseo_lite_fix_menu_item_url($url, $original_url, $_context) {
    if (preg_match('/(?:cyberseo_lite_upgrade_to_premium)$/i', $url)) {
        remove_filter('clean_url', 'cyberseo_lite_fix_menu_item_url', 10);
...
function csyn_main_menu() {
    if (function_exists('add_menu_page')) {
        ...
        add_submenu_page('cyberseo_lite', 'Upgrade To Premium', "<strong style=\"color: #FCB214;\">Get CyberSEO Pro</strong> ⭐", "manage_options", "cyberseo_lite_upgrade_to_premium", 'cyberseo_lite_upgrade_to_pro_menu');
        add_filter('clean_url', 'cyberseo_lite_fix_menu_item_url', 10, 3);
    }
}
...
function csyn_plugins_action_link($links) {
    $links[] = '<a href="https://www.cyberseo.net/" target="_blank"><b>Get CyberSEO Pro</b></a>';
    return $links;
}
